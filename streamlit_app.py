...
Calls retrieve_only()/generate() directly rather than the answer_question()
wrapper, so retrieval happens exactly once per strategy per question - each call
is a real, rate-limited Azure API request in live mode, so this UI must not
silently double it just to also display the raw retrieved chunks.
...
def _cached_answer(question: str, strategy: str, k: int):
    ...
    return answer, citations, latency_ms, docs
    ...
    for strat in list(STRATEGIES) if compare_all else [strategy]:
        ...
        try:
            with st.spinner(f"Retrieving + generating ({strat})..."):
                answer, citations, latency_ms, docs = _cached_answer(question, strat, k)
